...
async function run() {
  ...
  console.log(`V3 OpenRouter keys configured: ${configuredKeys.length}`);
  console.log(`V3 models configured: ${configuredModels.join(', ')}`);
  ...
  console.log(`V3 DOM analysis complete. Clickables: ${domAnalysis.clickable.length}, cards: ${domAnalysis.cards.length}`);
  ...
  try {
    ...
    console.log(`V3 frame collection complete. Frames: ${frames.length}`);
    ...
    const batchSize = Number(process.env.V3_MULTIMODAL_BATCH_SIZE || 6);
    const frameChunks = chunkItems(enrichedFrames, Math.max(1, batchSize));
    const chunkAnalyses = [];
    let reasoningStatus = { status: 'ok', reason: '' };
    console.log(`V3 multimodal chunk plan. Chunks: ${frameChunks.length}, chunk size: ${batchSize}`);
    ...
    for (let chunkIndex = 0; chunkIndex < frameChunks.length; chunkIndex += 1) {
      ...
      try {
        console.log(`V3 multimodal chunk start ${chunkIndex + 1}/${frameChunks.length}`);
        ...
        console.log(`V3 multimodal chunk complete ${chunkIndex + 1}/${frameChunks.length}`);
      } catch (err) {
        ...
        console.error(`V3 multimodal chunk failed ${chunkIndex + 1}/${frameChunks.length}: ${err.message}`);
        console.log(`V3 model snapshot after failure: ${JSON.stringify(getModelDebugSnapshot(configuredModels))}`);
      }
    }
    ...
    if (chunkAnalyses.length === 1) {
      ...
      try {
        console.log('V3 synthesis reasoning start');
        ...
        console.log('V3 synthesis reasoning complete');
      } catch (err) {
        console.error(`V3 Gemma synthesis failed: ${err.message}`);
        try {
          console.log('V3 synthesis compact retry start');
          ...
          console.log('V3 Gemma synthesis recovered with compact payload.');
        } catch (retryErr) {
          console.error(`V3 Gemma compact synthesis failed: ${retryErr.message}`);
    ...
    console.log('V3 analysis completed successfully.');
...
run().catch(err => {
  console.error(`V3 run failed: ${err.message}`);
  process.exitCode = 1;
});
